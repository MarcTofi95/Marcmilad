...
// particle-wave concept the client supplied, but deliberately toned way
// down: far fewer particles, a shallow/slow wave, a tight depth range and a
// camera that sits well back with only a hint of parallax, so nothing drifts
// toward the viewer. Renders behind the hero copy on the dark brand ground
// (#1D1D1D) with small, low-opacity gold (#E6C858) points instead of the
// original's dense white particle field on black.
//
...
export default function BrandWave() {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0x1d1d1d, 900, 2400);
    ...
    // A modest grid — calm, not a dense storm of particles.
    const AMOUNTX = 48;
    const AMOUNTY = 32;
    const SEPARATION = 46;
    ...
    function animate() {
      ...
      // Gentle parallax only — a small fraction of mouse offset, eased.
      targetX = mouseX * 40;
      targetY = mouseY * 20;
      camera.position.x += (targetX - camera.position.x) * 0.02;
      camera.position.y += (380 - targetY - camera.position.y) * 0.02;
      camera.lookAt(0, 0, 0);
