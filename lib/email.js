    ...
    '<body style="margin:0; padding:0; background:#DEDCD7; font-family:Arial, Helvetica, sans-serif;">' +
      '<div style="max-width:560px; margin:0 auto; padding:32px 20px;">' +
        '<div style="background:#FFFFFF; border-radius:16px; padding:36px 32px;">' +
          '<div style="font-size:11px; letter-spacing:.09em; text-transform:uppercase; color:#B08B00; font-weight:700;">TFA Commercial Productie</div>' +
...
export async function sendConfirmationEmail(brief) {
  ...
  const to = (brief.contactEmail && brief.contactEmail.trim()) || '';

  if (RESEND_API_KEY && to) {
    try {
      await sendViaResend(to, subject, html);
      console.log('[email] Confirmation email sent to ' + to + ' via Resend.');
      return;
    } catch (err) {
      console.error('[email] Resend send failed, falling back to DB row:', err.message);
    }
  } else if (RESEND_API_KEY && !to) {
    console.log('[email] No contactEmail on brief ' + brief.id + ' — saving as a DB row instead of sending.');
  }
